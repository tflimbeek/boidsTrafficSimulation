...
class MapBuilder():
    ...
    def __init__(self, file=None):
        ...
        if file is not None:
            self._import(os.getcwd() + "/maps/" + file)
    ...
    def _initBuffers(self):
        ...
        print(self.settingsData) #REMOVE
    ...
    def _onKeyEvent(self, window, key, scancode, action, mods):
        ...
        if key == glfw.KEY_UP and (action == glfw.PRESS or action == glfw.REPEAT):
            ...
            self.settingsData[0][3][1] -= 0.05
            self.centerPos[1] = (self.centerPos[1]-0.05)
        ...
        if key == glfw.KEY_LEFT and (action == glfw.PRESS or action == glfw.REPEAT):
            ...
            self.settingsData[0][3][0] += 0.05
            self.centerPos[0] = (self.centerPos[0]+0.05)
    ...
    def _onScrollEvent(self, window, x, y):
        if y<0 and self.settingsData[0][0][0]>0.001:
            #self.settingsData[0] = glm.scale(self.settingsData[0], glm.vec3(0.5)).to_list()
            self.settingsData[0][0][0]*=0.5
            self.settingsData[0][1][1]*=0.5
            self.settingsData[0][2][2]*=0.5
            self.settingsData[0][3][1]*=0.5
            self.settingsData[0][3][2]*=0.5
            
            self.scrollPos *= 0.5
            self.centerPos /= 0.5
        elif y>0 and self.settingsData[0][0][0] < 5:
